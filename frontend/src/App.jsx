...
const API_URL = import.meta.env.VITE_BOOK_API_URL || "http://13.59.20.23:8080/books";
...
function App() {
  ...
  const fetchComments = async (bookId, currentSort) => {
    if (!bookId) return;
    try {
      const res = await fetch(`http://13.59.20.23:8080/books/${bookId}/comments?sort=${currentSort}`);
      if (res.ok) {
        const data = await res.json();
        setComments(data);
      }
  ...
  const handleCommentSubmit = async (bookId, content) => {
    try {
      ...
      const res = await fetch(`http://13.59.20.23:8080/books/${bookId}/comments`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleCommentDelete = async (bookId, commentId) => {
    ...
    try {
      const authHeader = authToken?.startsWith("Bearer ") ? authToken : `Bearer ${authToken}`;
      const res = await fetch(`http://13.59.20.23:8080/books/${bookId}/comments/${commentId}`, {
        method: "DELETE",
  ...
  const handleCommentLike = async (bookId, commentId) => {
    try {
      const authHeader = authToken?.startsWith("Bearer ") ? authToken : `Bearer ${authToken}`;
      const res = await fetch(`http://13.59.20.23:8080/books/${bookId}/comments/${commentId}/like`, {
        method: "POST",
        headers: { Authorization: authHeader },
      });
